    ...
    for sql_regex in sql_regex_list:
        ...
        for param in tmp_params:
            ...
            if not sql_param.is_function:
                ...
                mybatis_param.python_type = __get_regex_value(
                    "(\s*python_type\s*=\s*)(?P<python_type>\w+)?", param, "python_type"
                )
                mybatis_param.type_handler = __get_regex_value(
                    "(\s*type_handler\s*=\s*)(?P<type_handler>\w+)?", param, "type_handler"
                )
